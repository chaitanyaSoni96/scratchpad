...
`skill/SKILL.md` (Agent Skills format) restates the store's rules for agents and is the single agent-facing contract — if publish/name/create-only semantics, the notes verbs (`resolve`/`reply`, never `create`/`edit`/`delete`/`reopen`), or CLI flags change, update the skill and `README.md` together, and re-run `make install-skill` to push the copies in `~/.claude/skills` and `~/.pi/agent/skills` (they are copies, not symlinks — an edit in the repo does nothing until you re-run it).
